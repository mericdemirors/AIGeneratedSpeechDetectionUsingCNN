# Trains feature extraction SVMs on a dataset, and saves the evaluation scores in a txt
...
model_serial_number = "feat_ext_" + model_type + "_" + os.path.split(root_dir)[1] + "_" + datetime.now().strftime("%m_%d_%H_%M_%S")
...
def load_batch(Xs, Ys):
    images = [np.load(path) for path in tqdm(Xs, desc="loading images")]
    return images, Ys
