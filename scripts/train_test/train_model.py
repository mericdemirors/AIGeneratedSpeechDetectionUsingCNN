# Trains a model on a dataset, and saves the evaluation scores in a txt
...
# dimension reduction is done if transform keyword is used
...
for epoch in range(NUM_OF_EPOCHS):
    ...
    # if there is no previous epoch validation or a better one is found
    if min_validation_loss is None or validation_loss < min_validation_loss:
        # reset the early stopping, update the best one
        early_stop_step = 0
